 ...
 *
 * Layout:
 * - Mobile: scroll natural (min-h-screen), capa oculta
 * - Desktop: altura fixa (h-screen), capa visível, descrição scrollável
 */
function Details() {
  const { id } = useParams();
  ...
  return (
    <div className="min-h-screen lg:h-screen flex flex-col bg-slate-100 dark:bg-slate-950">
      <Header />

      <main className="flex-1 lg:overflow-hidden">
        <div className="lg:h-full container mx-auto px-4 py-6 max-w-6xl">
          <div className="grid grid-cols-1 lg:grid-cols-[minmax(280px,380px)_1fr] gap-6 lg:h-full">
            {/* Coluna da CAPA */}
            {/* Mobile: imagem grande acima do card | Desktop: coluna lateral */}
            <div className="flex justify-center lg:justify-start lg:items-start lg:h-full lg:overflow-hidden">
              <img
                src={getBestBookCover(volumeInfo.imageLinks)}
                alt={volumeInfo.title}
                className="w-48 lg:w-full lg:max-w-95 lg:max-h-full object-contain rounded-lg shadow-2xl"
                onError={(e) => {
                  e.target.src =
            ...
            {/* Coluna da INFO */}
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-lg p-4 lg:p-6 flex flex-col lg:h-full lg:overflow-hidden">
              <div className="shrink-0 mb-4">
                {/* Título e Autor (mesmo layout mobile e desktop) */}
                <div className="mb-4">
              ...
              {/* Descrição (scrollável no desktop, natural no mobile) */}
              <div className="lg:flex-1 lg:overflow-y-auto pr-2 glass-scrollbar">
